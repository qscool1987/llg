...
mPausedTime(0), mPrevTime(0), mCurrTime(0), mStopped(false), mFrequency(-1.0f), mAcculate(0.0)
{
...
void GameTimer::Tick()
{
	...
	mAcculate += mDeltaTime;
	if (mAcculate > 1.0) {
		mAcculate = 0.0;
		mFrequency = static_cast<float>(1 / mDeltaTime);
	}
}
